...
// Production-grade error handling for hosting stability
process.on('uncaughtException', (error) => {
    console.error('🚨 Uncaught Exception:', error.message);
    console.error('📍 Stack trace:', error.stack);
    console.log('🔄 Attempting graceful recovery...');
    
    // Attempt to save any active sessions before potential restart
    try {
        if (typeof global.activeSessions !== 'undefined' && global.activeSessions && global.activeSessions.size > 0) {
            console.log(`💾 Preserving ${global.activeSessions.size} active sessions...`);
            if (typeof global.saveSessionData === 'function') {
                global.saveSessionData().catch(err => console.error('Failed to save sessions:', err));
            }
        }
    } catch (err) {
        console.error('Error accessing session data during recovery:', err.message);
    }
});

process.on('unhandledRejection', (reason, promise) => {
    console.error('🚨 Unhandled Rejection detected:', reason?.message || reason);
    console.error('📍 Stack trace:', reason?.stack);
    // Log but continue - hosting environments should remain stable
});
...
const client = new Client({
    ...
    // Production-optimized sweepers for memory management
    sweepers: {
        messages: {
...
const emptyChannelTimestamps = new Map(); // Track empty channels for cleanup

// Make session data globally accessible for error handlers
global.activeSessions = activeSessions;

// Make session data globally accessible for error handlers
global.activeSessions = activeSessions;
global.saveSessionData = null; // Will be assigned later

// Memory optimization - limit map sizes for hosting stability
const MAX_CACHE_SIZE = 1000;

function cleanupOldCacheEntries(map, maxSize = MAX_CACHE_SIZE) {
    if (map.size > maxSize) {
...
// Make saveSessionData globally accessible for error handlers
global.saveSessionData = saveSessionData;
...
setInterval(async () => {
    try {
        ...
        // Memory optimization - cleanup cache maps for hosting stability
        cleanupOldCacheEntries(gameCategories);
        cleanupOldCacheEntries(emptyChannelTimestamps);
        cleanupOldCacheEntries(voiceChannelOperations, 100);
    } catch (error) {
...
async function createLfgVoiceChannel(guild, user, gameData, category) {
    try {
        ...
        if (!botMember?.permissions.has([PermissionFlagsBits.ManageChannels, PermissionFlagsBits.Connect])) {
            console.error('❌ Bot missing permissions. Required: Manage Channels, Connect');
            throw new Error('Bot lacks required permissions to create voice channels. Please ensure the bot has "Manage Channels" and "Connect" permissions.');
        }
...
async function registerCommands() {
    const commands = [
        ...
        new SlashCommandBuilder()
            .setName('quickjoin')
            .setDescription('🚀 Instantly join any available LFG session for your game!')
            .addStringOption(option =>
                option.setName('game')
            ...
            .addStringOption(option =>
                option.setName('gamemode')
                    .setDescription('🎯 Choose your preferred gamemode')
                    .setRequired(true)
                    .setAutocomplete(true))
            ...
            if (focusedOption.name === 'gamemode') {
                ...
                if (game && SUPPORTED_GAMES[game]) {
                    ...
                    const modes = SUPPORTED_GAMES[game].modes.filter(mode => 
                        ...
                        searchTerm.includes(mode.toLowerCase())
                    );
                    
                    // If no matches and user typed something, show all modes
                    const modestoShow = modes.length > 0 ? modes : SUPPORTED_GAMES[game].modes;
                    
                    await interaction.respond(
                        modestoShow.slice(0, 25).map(mode => ({ name: mode, value: mode }))
                    );
                } else {
                    await interaction.respond([]);
                }
...
async function handleLfgCommand(interaction) {
    ...
    // Validate gamemode with case-insensitive matching
    const validMode = gameData.modes.find(mode => 
        mode.toLowerCase().trim() === gamemode.toLowerCase().trim()
    );
    ...
    // Use the properly formatted mode name
    const normalizedGamemode = validMode;
    ...
    try {
        ...
        voiceChannel = await createLfgVoiceChannel(guild, user, gameData, category);
...
async function getOrCreateGameCategory(guild, gameKey, gameName) {
    ...
    if (!category) {
        // Verify bot has permissions to create categories
        const botMember = guild.members.cache.get(client.user.id);
        if (!botMember?.permissions.has(PermissionFlagsBits.ManageChannels)) {
            console.error('❌ Bot missing "Manage Channels" permission for creating categories');
            throw new Error('Bot lacks required permissions to create categories. Please ensure the bot has "Manage Channels" permission.');
        }
...
async function handleJoinLfg(interaction) {
    try {
        // Defer the reply immediately to prevent "thinking forever"
        ...
        if (session.currentPlayers.includes(interaction.user.id)) {
            ...
            if (session.creator === interaction.user.id) {
                // Creator clicked join - provide helpful info without "Already in Team" message
                const creatorEmbed = new EmbedBuilder()
                    .setColor(0x3498db)
                    .setTitle('👑 **Session Manager**')
                    .setDescription(`**You're the creator of this ${session.game} session**\n\n👥 **Team:** ${session.currentPlayers.length}/${session.playersNeeded}\n🔊 **Voice:** <#${session.voiceChannel}>\n⏰ **Status:** ${session.status === 'confirming' ? 'Waiting for confirmations' : 'Looking for more players'}\n\n*Use \`/endlfg\` to end this session when you're done.*`)
                    .setTimestamp();
                
                console.log(`Session creator ${interaction.user.displayName} clicked join on their own session`);
                
                return await interaction.editReply({ 
                    embeds: [creatorEmbed]
                });
            } else {
                // Regular user already in session, show status with leave option
        ...
        if (session.currentPlayers.length >= session.playersNeeded) {
            ...
            return await interaction.editReply({ embeds: [fullEmbed] });
        }
    ...
    if (session.currentPlayers.length === session.playersNeeded) {
        ...
        // Since we deferred the reply, we need to update the original message differently
        try {
            // First, send a confirmation to the user
            await interaction.editReply({ 
                content: '✅ Team is full! Starting confirmation process...'
            });
            
            // Update the original message via the channel
            ...
            if (channel && session.messageId) {
                try {
                    ...
                    console.error('Could not update original message:', messageError);
                }
            }
            
            // Send confirmation ping to all players
            if (channel) {
                await channel.send({ 
                    content: `${playerPings}`,
                    embeds: [confirmEmbed],
            ...
            console.error('Error handling full team confirmation:', interactionError);
            await interaction.editReply({ 
                content: '✅ Successfully joined! Team is now full and starting confirmation process.'
            });
        }
...
async function handleConfirmation(interaction) {
    ...
    if (!session.currentPlayers.includes(interaction.user.id)) {
        const notInSessionEmbed = new EmbedBuilder()
            ...
            .setTimestamp();
        return await interaction.reply({ embeds: [notInSessionEmbed], ephemeral: true });
    }
    ...
    if (session.status !== 'confirming') {
        const notConfirmingEmbed = new EmbedBuilder()
            ...
            .setTimestamp();
        return await interaction.reply({ embeds: [notConfirmingEmbed], ephemeral: true });
    }
    ...
    if (session.confirmedPlayers.includes(interaction.user.id)) {
        const alreadyConfirmedEmbed = new EmbedBuilder()
            ...
            .setTimestamp();
        return await interaction.reply({ embeds: [alreadyConfirmedEmbed], ephemeral: true });
    }
    ...
    if (session.confirmedPlayers.length === session.currentPlayers.length) {
        ...
        await interaction.reply({ content: '✅ Confirmed! Waiting for other players...', ephemeral: true });
    }
}
...
async function handleDecline(interaction) {
    ...
    if (!session || !session.currentPlayers.includes(interaction.user.id)) {
        return await interaction.reply({ content: '❌ You are not part of this LFG!', ephemeral: true });
    }
...
async function handleLeaveLfg(interaction) {
    ...
    if (!session) {
        return await interaction.reply({ content: '❌ This LFG session is no longer active!', ephemeral: true });
    }
    
    if (!session.currentPlayers.includes(interaction.user.id)) {
        return await interaction.reply({ content: '❌ You are not in this LFG session!', ephemeral: true });
    }
    ...
    try {
        ...
        await interaction.reply({ content: '✅ You left the LFG session.', ephemeral: true });
...
// Production-grade HTTP server with comprehensive health checks
const http = require('http');
const port = process.env.PORT || 3000;

const server = http.createServer(async (req, res) => {
    // Enable CORS for web panels
...
async function handleQuickJoinCommand(interaction) {
    try {
        ...
        const availableSessions = Array.from(activeSessions.values()).filter(session => 
            ...
            session.creator !== userId // Don't show user's own sessions
        );
        
        if (availableSessions.length === 0) {
            const gameDisplayName = SUPPORTED_GAMES[game]?.name || game;
            return interaction.reply({
                content: `🔍 **No available ${gameDisplayName} - ${gamemode} sessions found!**\n\n` +
                        `🎮 There are currently no open **${gameDisplayName} ${gamemode}** sessions looking for players\n` +
                        `💡 Create your own session with \`/lfg\` or try a different game/mode\n` +
                        `⏰ Check back in a few minutes for new sessions!`,
                flags: 64
            });
        }
        
        // Sort sessions by creation time (newest first) and player count (most needed first)
        availableSessions.sort((a, b) => {
            const aNeeded = a.playersNeeded - a.currentPlayers.length;
            const bNeeded = b.playersNeeded - b.currentPlayers.length;
            
            if (aNeeded !== bNeeded) {
                return bNeeded - aNeeded; // Sessions needing more players first
            }
            return b.createdAt - a.createdAt; // Then by newest
        });
        
        // Join the best available session
        ...
        const sessionFull = targetSession.currentPlayers.length >= targetSession.playersNeeded;
        ...
        try {
            ...
            const message = channel && await channel.messages.fetch(targetSession.messageId).catch(() => null);
            
            if (message) {
                const updatedEmbed = await createLfgEmbed(targetSession, sessionFull);
                const components = sessionFull ? [] : [createJoinButton(targetSession.id)];
                await message.edit({ embeds: [updatedEmbed], components });
            }
        } catch (error) {
            console.error('Error updating LFG message after quick join:', error);
        }
        
        // Add user to voice channel if it exists
        try {
            const guild = client.guilds.cache.get(targetSession.guildId);
            const voiceChannel = guild?.channels.cache.get(targetSession.voiceChannel);
            ...
            console.error('Error adding user to voice channel:', error);
        }
        
        // Create response embed
        const playersNeeded = targetSession.playersNeeded - targetSession.currentPlayers.length;
        const embed = new EmbedBuilder()
            .setColor('#00ff88')
            .setTitle('🚀 Quick Join Successful!')
            .setDescription(
                `🎮 **Game:** ${gameDisplayName}\n` +
                `🎯 **Mode:** ${targetSession.gamemode}\n` +
                `👥 **Players:** ${targetSession.currentPlayers.length}/${targetSession.playersNeeded}\n` +
                (targetSession.info ? `📝 **Info:** ${targetSession.info}\n` : '') +
                (sessionFull ? 
                    '\n🎉 **Team is now full!** Check your voice channel to connect with teammates!' :
                    `\n⏳ **Still need ${playersNeeded} more player${playersNeeded === 1 ? '' : 's'}**`
                )
            )
            .setFooter({ text: 'Ready to game! Good luck and have fun! 🎮' })
            .setTimestamp();
        
        // Add voice channel info if available
        if (targetSession.voiceChannel) {
            const guild = client.guilds.cache.get(targetSession.guildId);
            const voiceChannel = guild?.channels.cache.get(targetSession.voiceChannel);
            if (voiceChannel) {
                embed.addFields({
        ...
        // If session is full, handle completion
        if (sessionFull) {
            console.log(`🎉 Session #${targetSession.id.slice(-6)} completed via Quick Join! All ${targetSession.playersNeeded} players found.`);
